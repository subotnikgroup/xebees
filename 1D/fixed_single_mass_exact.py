...
@timer
def solve_exact(NR, Nr, R, r, M, m, g_1,g_2,num_states=5):
...
@timer
def build_preconditioner(Tr, TR, Vgrid, nguess=1):
    NR, Nr = Vgrid.shape
...
@timer
def solve_davidson(NR, Nr, R, r, M, m, num_state=10, g_1=1,g_2=1, verbosity=2,
                   iterations=1000,
                   max_subspace=1000,
                   threads=16,
                   guessfile=None):
...
if __name__ == '__main__':
    ...
    # Grid setup
    R = np.linspace(-4, -2, args.NR) * ANGSTROM_TO_BOHR
    r = np.linspace(-3, 0, args.Nr) * ANGSTROM_TO_BOHR


    conv, e_approx, evecs = solve_davidson(args.NR, args.Nr, R, r, M, m, num_state=args.k, g_1=args.g_1, g_2=args.g_2,
    ...
    if args.exact_diagonalization:
        e_exact = solve_exact(args.NR, args.Nr, R, r, M, m, g_1=args.g_1,g_2=args.g_2)
        print("Exact:", e_exact)
