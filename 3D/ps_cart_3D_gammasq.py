...
#                ddz1 = H.ddz1[None, None, :, :]
#
#                test2 =xp.einsum('x,xybz,x,xbYz,BxYz->Bxyz', H.x, ddy1, H.x, ddy1, xdavnew, optimize=True)
#
#            
...
#                
#    
#                px_old =  xp.kron(xp.kron(H.ddx1, xp.eye(Ny)), xp.eye(Nz))   
...
#                py_old =  xp.kron(xp.kron(xp.diag(H.x), H.ddy1), xp.eye(Nz))
#                #rx2 = xp.kron(xp.kron(xp.eye(Nx), xp.eye()), xp.eye(Nz))
#                test1 = (py_old@(py_old@xdot))   
#                #test1= ((H.M_2**2*gammasq1x_old)+(H.M_1**2*gammasq2x_old)-(H.M_1*H.M_2*gamma1x2x_old)-(H.M_1*H.M_2*gamma2x1x_old))/(H.M_1+H.M_2)**2
#                
#                
#                 
#                print("diff",xp.sum((test1-test2.flatten())**2))
#                       
#                exit()
#rng = xp.random.default_rng(0)
                    ...
                    #Hu = Hx_ps(u.reshape(1, -1)).reshape(shape)
                    #Hv = Hx_ps(v.reshape(1, -1)).reshape(shape)
