### command small enough to fit on della mig gpu 10GB limit:
### python fixed_center_of_mass_exact_cupy.py -g_1 1 -g_2 10 -M_1 30 -M_2 30 -R 11 -r 22 -g 24 --verbosity 10 --preconditioner naive --iterations 300
###
...
import linalg_helper_cupy as lib
#from pyscf import lib
import potentials
from constants import *
from hamiltonian_cupy import  KE, KE_FFT, KE_Borisov
...
class Hamiltonian:
    ...
    def __init__(self, args):
        ...
        self.r     = cp.linspace(r_max    /args.Nr, r_max, args.Nr)
        self.r_lab = cp.linspace(r_max_lab/args.Nr, r_max_lab, args.Nr)
        self.R     = cp.linspace(*R_range,     args.NR)
        self.R_lab = cp.linspace(*R_range_lab, args.NR)
        ...
        # all over the place
        self.g = cp.linspace(0, 2*np.pi, args.Ng, endpoint=False)

        self.axes = (self.R, self.r, self.g)

        self.R_grid, self.r_grid, self.g_grid = cp.meshgrid(self.R, self.r, self.g, indexing='ij')
        ...
        self.P  = cp.fft.fftshift(cp.fft.fftfreq(args.NR, dR)) * 2 * np.pi
        self.p  = cp.fft.fftshift(cp.fft.fftfreq(args.Nr, dr)) * 2 * np.pi
        self.pg = cp.fft.fftshift(cp.fft.fftfreq(args.Ng, dg)) * 2 * np.pi
        ...
        self.ddR2    = cp.asarray(KE(args.NR, dR, bare=True, cyclic=False) + np.diag(1/4/self.R**2))
        self.ddr2, _ = KE_Borisov(self.r, bare=True)
    ...
    def Tx(self, x):
        ...
        ke += cp.einsum('Rrg,rs->Rsg', xa, self.ddr2)  # ∂²/∂r²

        #  ∂²/∂γ² + 1/4 terms
        keg  = cp.einsum('Rrg,gh->Rrh', xa, self.ddg2)  # ∂²/∂γ²
        ke += (self.Rinv2 + self.rinv2)*keg              # (1/R² + 1/r²) (∂²/∂γ²)

        # Angular Kinetic Energy J terms
        if self.J != 0:
            keg  = xa*self.J**2                                        #  J²
            keg += 2j*self.J*cp.einsum('Rrg,gh->Rrh', xa, self.ddg1)  #  J² + 2Ji ∂/∂γ
    ...
    def buildDiag(self):
        ...
        if self.J != 0:
            ke = self.Rinv2 * (
                self.J**2 +
                2*self.J*cp.ones(self.shape) * cp.diag(self.ddg1)[None, None, :]
            )
