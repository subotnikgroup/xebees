...
if __name__ == '__main__':
    ...
    Gammasqtoty = xp.zeros([NR,Nelec,Nelec],dtype=complex)
    Gammatotx = xp.zeros([NR,Nelec,Nelec],dtype=complex)
    Gammatoty = xp.zeros([NR,Nelec,Nelec],dtype=complex)
    ...
    for i in range(H.shape[0]):
        ...
        with timer_ctx("build gamma"):
            gammaetf1x, gammaetf1y, gammaetf2x, gammaetf2y, gammaerf1y, gammaerf2y = Gamma_etf_erf_cart(H.R[i],H.x_grid,H.y_grid,H.ddx,H.ddy,H.M_1,H.M_2,H.mu12,r1e2,r2e2)
        
        Gammatotx[i] = (H.M_2*gammaetf1x-H.M_1*gammaetf2x)/(H.M_1+H.M_2)
        Gammatoty[i] = (H.M_2*(gammaetf1y+gammaerf1y)-H.M_1*(gammaetf2y+gammaerf2y))/(H.M_1+H.M_2)
        gammasq1x = xp.dot(gammaetf1x,gammaetf1x)
        gammasq2x = xp.dot(gammaetf2x,gammaetf2x)
        gammasq1y = xp.dot((gammaetf1y+gammaerf1y),(gammaetf1y+gammaerf1y))
        gammasq2y = xp.dot((gammaetf2y+gammaerf2y),(gammaetf2y+gammaerf2y))

        Gammasqtotx[i] = ((H.M_2**2*gammasq1x)+(H.M_1**2*gammasq2x)-(H.M_1*H.M_2*xp.dot(gammaetf1x,gammaetf2x))-(H.M_1*H.M_2*xp.dot(gammaetf2x,gammaetf1x)))/(H.M_1+H.M_2)**2
        Gammasqtoty[i] = ((H.M_2**2*gammasq1y)+(H.M_1**2*gammasq2y)-(H.M_1*H.M_2*xp.dot(gammaetf1y,gammaetf2y))-(H.M_1*H.M_2*xp.dot(gammaetf2y,gammaetf1y)))/(H.M_1+H.M_2)**2 
        
        #H_gamma[i] += (Gammatotx*xp.cos(H.theta)+Gammatoty*xp.sin(H.theta))
        #H_gamma[i] += (Gammatotx*xp.cos(H.theta)+Gammatoty*xp.sin(H.theta))
        
    index_pairs = [(j, k, Htot_bo_test, gammacoeff_R, gammacoeff_theta,Gammatotx[j],Gammatoty[j],Gammasqtotx[j], Gammasqtoty[j]) for j in range(NR) for k in range(NR)]
    threadctl = ThreadpoolController()
    blasthreads = args.t
